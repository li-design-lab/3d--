...
道路页面还支持按桩号关联的要素层：能见度、降水、路面状态、2 米气温、2 米地温和风速。要素值、单位、等级、等级描述和颜色会在选中桩点详情中显示，并以颜色标记叠加在线路上。要素接口数据不含坐标，缺失桩点不会插值；本机输入保存在 `.private/road-elements.json`，公开演示版使用独立生成的虚构要素。
