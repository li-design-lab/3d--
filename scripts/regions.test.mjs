...
test('store caches success, retries failure and rejects missing levels',async()=>{
  ...
  let calls=0;
  globalThis.fetch=async()=>{calls++;if(calls===1)return {ok:false,status:404};return {ok:true,json:async()=>data};};
  try{
    ...
    await assert.rejects(store.get('510100'),/404/);
...
test('slow requests abort instead of spinning forever',async()=>{
  ...
  try{await assert.rejects(readJson('slow.json',10,1),/超时/);}finally{globalThis.fetch=original;}
});
...
test('township polygons with holes remain intact and invalid coordinates fail',()=>{
 ...
 test('transient failures retry automatically and reload cached failures',async()=>{
 const original=globalThis.fetch;let calls=0;
 globalThis.fetch=async(_url,options)=>{calls++;if(calls===1)throw TypeError('network failure');if(calls===2)return {ok:false,status:503};assert.equal(options.cache,'reload');return {ok:true,json:async()=>({ok:true})};};
 try{assert.deepEqual(await readJson('retry.json'),{ok:true});assert.equal(calls,3);}finally{globalThis.fetch=original;}
 });
